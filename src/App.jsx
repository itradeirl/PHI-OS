...
export default function PHIOS() {
  ...
  const fetchLiveData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setDataError(quotes.error || "Market may be closed or data temporarily unavailable.");
        setLoading(false);
        return;
      }

      const map = {};
      for (const s of BASE_WATCHLIST) {
  ...
  const generateMorningBrief = async () => {
    ...
    setBriefError(null);
    try {
      // Fetches the most recent brief already generated by the real
                      ...
                      {morningBrief.text.split("\n\n").map((section, i) => {
                        ...
                        return (
                          <div key={i} style={{ marginBottom: 8, padding: "8px 10px", background: "#0a0e1a", borderRadius: 6, borderLeft: "2px solid #C9A84C33" }}>
                            ...
                            <div style={{ fontSize: 11, color: "#94a3b8", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{body || (!isHeader ? section : "")}</div>
                          </div>
                        );
                      })}
