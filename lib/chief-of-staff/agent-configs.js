...
PHI FIBONACCI EXTENSION ZONES (extensions — exits, open positions only):
...
127.2% → First take-profit zone, trim candidate
161.8% → Golden ratio extension, strong trim signal
200.0% → Round-number target, many traders exit here
261.8% → Extended/euphoric move, high trim conviction

For each OPEN position specifically (not the full watchlist), if the tool
output shows aboveSwingHigh is true:
- Report nearestExtensionZone, nearestExtensionPrice, distanceFromExtension
- Flag AT EXTENSION TARGET or APPROACHING EXTENSION TARGET as a trim/exit
  signal, distinct from and never confused with a retracement BUY ALERT
- Do not invent an exit signal for a position still below its swing high —
  say plainly that no extension target applies yet
`,
  ...
  getPrompt: (trigger, config) => `
    ...
    Then, separately, run Fibonacci EXTENSION analysis on Indygo's current
    open positions only:
    ${config.portfolio.positions.map(p => `${p.ticker} (${p.shares} sh @ $${p.avgCost} avg cost)`).join(", ")}

    For each open position, use the same tiingo_candles call's
    aboveSwingHigh/nearestExtensionZone/nearestExtensionPrice/
    distanceFromExtension/extensionStatus fields (no extra tool call needed
    if you already pulled it for the watchlist pass above) to flag any
    position that has reached or is approaching an extension-based
    ...
    - PROFIT-TAKING TARGETS section: any open position AT or APPROACHING an
      extension zone, with the specific level and price. If no open
      position is above its swing high yet, say so plainly instead of
