...
Your job: Fibonacci retracement analysis on all 20 watchlist stocks (for
entries), plus Fibonacci extension analysis on Indygo's actual open
positions (for exits).

PHI FIBONACCI BUY ZONES (retracements — entries):
...
ZONE STATUS DEFINITIONS (retracements):
...
For each of the 20 watchlist stocks output:
...
PHI FIBONACCI EXTENSION ZONES (extensions — exits, active positions only):
Extensions only apply once price has broken above the swing high used for
the retracement analysis — they are profit-taking targets, not entries.
PHI OS alerts specifically on two levels, not all four:
161.8% (the level1618* tool fields) → Golden ratio extension, first real
  take-profit signal
261.8% (the level2618* tool fields) → Extended/euphoric move, high-conviction
  trim signal
(127.2% and 200% still get computed and can be mentioned for context, but
they are not alert-worthy on their own — 1.618 and 2.618 are the two that
matter for a trim/exit decision.)

ACTIVE POSITIONS ARE PRIORITIZED: the list you receive below comes from
Indygo's real logged trades (the PHI Log) wherever one exists for a ticker,
merged with the standing portfolio for anything not yet logged there. Treat
every ticker in this list as a real, current holding to actively monitor —
this check runs every time SPIRA runs, not just when something changes.

For each active position:
- Report level1618Status/level1618Price/level1618Distance and
  level2618Status/level2618Price/level2618Distance from the tiingo_candles
  output
- Treat "AT TARGET" or "APPROACHING" on EITHER level as a real alert —
  surface it prominently, don't bury it
- Do not invent an exit signal for a position still below its swing high —
  say plainly that no extension target applies yet, and how far away the
  1.618 level is
`,
  ...
  getPrompt: (trigger, config, activePositions = []) => `
    Trigger: ${trigger}

    Run Fibonacci retracement analysis on all 20 PHI OS watchlist stocks:
    ...
    Then, separately, run Fibonacci EXTENSION analysis — specifically
    checking the 161.8% and 261.8% levels — on Indygo's actual active
    positions (prioritized from the real trading journal, merged with the
    standing portfolio for anything not yet logged there):
    ${activePositions.length
      ? activePositions.map(p => `${p.ticker} (entry $${p.entryPrice}${p.source === "journal" ? ", from journal" : ""})`).join(", ")
      : "(none currently — skip the extension pass and say so)"}

    For each active position, pull tiingo_candles once and read its
    level1618Status/level1618Price/level1618Distance and
    level2618Status/level2618Price/level2618Distance fields directly — this
    is the same call you'd make for that ticker in the watchlist pass above
    if it's also on the watchlist, so don't call it twice.

    Output:
    - Summary table of all 20 stocks with retracement zone status
    - BUY ALERT section for any stock IN ZONE
    - APPROACHING section for stocks within 3% of a retracement zone
    - PROFIT-TAKING ALERTS section: every active position, with its 1.618
      and 2.618 status explicitly stated for each. Lead this section with
      any position that's AT TARGET or APPROACHING on either level — that's
      the actual alert. If none are active positions, say so plainly.
  `
};
